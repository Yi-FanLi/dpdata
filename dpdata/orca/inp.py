...
# https://github.com/deepmodeling/dpgen/blob/0767dce7cad29367edb2e4a55fd0d8724dbda642/dpgen/generator/lib/orca.py#L1-L190
...
def _crd2frag(symbols: list[str], crds: np.ndarray) -> tuple[int, list[int]]:
    ...
    PBC support is removed from this method as ORCA does not support PBC calculation.
...
def make_orca_input(
    ...
    keywords : str or list[str]
        ORCA keywords, e.g. force b3lyp/6-31g**. If a list,
        run multiple steps
    ...
    if not sys_data.get("nopbc", False):
        ...
        for ii in range(3):
            # use TV as atomic symbol, see https://orca.com/pbc/
